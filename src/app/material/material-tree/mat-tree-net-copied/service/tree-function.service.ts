...
export class TreeFunctionService {
  ...
  findNodeMaxId(node: TreeData[]): number {
    ...
    for (const element of flatArray) {
      if (typeof element.Id === 'number') ids.push(element.Id);
    }
    return ids.length ? Math.max(...ids) : 0;
  }

  findPosition(id: number | null, data: TreeData[]): number | null {
  ...
  findFatherNode(id: number | null, data: TreeData[]): readonly [TreeData, number] | false {
